...
client.on("ready", () => {
    ...
    console.log('Version ' + process.env.GK_VERSION)
    console.log(`----------------------------------`);
...
client.on("message", async message => {
    ...
    if (cmd === 'gkemail') {
        ...
        con.query(`SELECT whitelist FROM whitelist WHERE server_id = '${message.guild.id}'`, (err, rows) => { // find server submitted whitelist
            // let db_whitelist = ["rows[0].whitelist"];
            if (rows.length > 0) {
        ...
        con.query(`SELECT auth_code FROM main WHERE server_id = '${message.guild.id}' AND user_id = '${message.author.id}'`, (err, rows) => {
            if(err) throw err;
            let code = rows[0].auth_code;
            console.log('db code: ' + code);

            if(user_input === code) { // User submitted verification equals to one in database
